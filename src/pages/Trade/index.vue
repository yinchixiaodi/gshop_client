  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      >
        <span
          class="username "
          :class="{ selected: address === selectAddress }"
...
export default {
  ...
  mounted() {
    this.$store.dispatch("getTraderInfo");
  },
  methods: {
    async submitOrder() {
      const { tradeNo, detailArrayList } = this.tradeInfo;
      const { userAddress, consignee, phoneNum } = this.selectAddress;
      const orderInfo = {
        consignee,
      ...
      // console.log(this.tradeInfo, tradeNo, orderInfo);
      if (result.code === 200) {
  ...
  watch: {
    "tradeInfo.userAddressList"(value) {
      const selectAddress = value.find((item) => item.isDefault === "1");
