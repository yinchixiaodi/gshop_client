    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          ...
          />
          <!-- 小图列表 -->
          <ImageList @changeCurrent="handelChangeCurrent" />
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
            ...
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd
                  ...
                  :key="value.id"
                  :class="{
                    active: value.isChecked === '1',
                  }"
                  @click="indexChange(value, spuSaleAttr.spuSaleAttrValueList)"
                >
                  <!-- @click="indexChange(value, index1, index2)" -->
                  <!--  -->
...
export default {
  ...
  mounted() {
    this.$store.dispatch("getProduct", this.$route.params.skuId);
  },
  methods: {
    handelChangeCurrent(index) {
      this.currentIndex = index;
    },
    indexChange(value, arr) {
      if (value.isChecked !== 1) {
        arr.forEach((v) => (v.isChecked = "0"));
        value.isChecked = "1";
      }
    },
    // 点击添加到购物车的回调函数
    async addCart() {
      ...
      // 方法二 ：1
      /* const errorMsg = await this.$store.dispatch("getAddToCart2", {
        skuId,
        skuNum,
      });
      if (errorMsg) {
    ...
    // 在组件中分发异步action之后, 如何知道是成功了还是失败了?
    // 方法一: 定义一个回调函数
    /* callback(errorMsg) {
      if (errorMsg) {
        // 如果errorMsg有值，代表加入购物车出错
        alert(errorMsg);
      } else {
        alert("添加购物车成功！");
      }
